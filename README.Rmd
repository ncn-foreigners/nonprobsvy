...
+------------+------------+------------+------------+-----------------+
| Sample     |            | Auxiliary  | Target     | Design          |
|            |            | variables  | variable   | ($\             |
|            |            | $\bold     | $Y$        | boldsymbol{d}$) |
|            |            | symbol{X}$ |            | or calibrated   |
|            |            |            |            | ($\             |
|            |            |            |            | boldsymbol{w}$) |
|            |            |            |            | weights         |
+============+===========:+:==========:+:==========:+:===============:+
| $S_A$      | 1          | $\         | $\         | ?               |
| (non-pr    |            | checkmark$ | checkmark$ |                 |
| obability) |            |            |            |                 |
+------------+------------+------------+------------+-----------------+
|            | ...        | $\         | $\         | ?               |
|            |            | checkmark$ | checkmark$ |                 |
...
|                                  |                                  |
+----------------------------------+----------------------------------+
| Mass imputation based on         | \`\`\`{r, eval = FALSE} nonprob( |
| regression imputation            | outcome = y \~ x1 + x2 + ... +   |
|                                  | xk, data = nonprob, pop_totals = |
|                                  | c(\`(Intercept)\`= N, x1 =       |
|                                  | tau_x1, x2 = tau_x2, ..., xk =   |
|                                  | tau_xk), method_outcome = "glm", |
|                                  | family_outcome = "gaussian" )    |
|                                  | \`\`\`                           |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
| Inverse probability weighting    | \`\`\`{r, eval = FALSE} nonprob( |
|                                  | selection = \~ x1 + x2 + ... +   |
|                                  | xk, target = \~ y, data =        |
|                                  | nonprob, pop_totals =            |
|                                  | c(\`(Intercept)\` = N, x1 =      |
|                                  | tau_x1, x2 = tau_x2, ..., xk =   |
|                                  | tau_xk), method_selection =      |
|                                  | "logit" ) \`\`\`                 |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
| Inverse probability weighting    | \`\`\`{r, eval = FALSE} nonprob( |
| with calibration constraint      | selection = \~ x1 + x2 + ... +   |
|                                  | xk, target = \~ y, data =        |
|                                  | nonprob, pop_totals =            |
|                                  | c(\`(Intercept)\`= N, x1 =       |
|                                  | tau_x1, x2 = tau_x2, ..., xk =   |
|                                  | tau_xk), method_selection =      |
|                                  | "logit", control_selection =     |
|                                  | control_sel(est_method = "gee",  |
|                                  | gee_h_fun = 1) ) \`\`\`          |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
| Doubly robust estimator          | \`\`\`{r, eval = FALSE} nonprob( |
|                                  | selection = \~ x1 + x2 + ... +   |
|                                  | xk, outcome = y \~ x1 + x2 +     |
|                                  | …, + xk, pop_totals =            |
|                                  | c(\`(Intercept)\` = N, x1 =      |
|                                  | tau_x1, x2 = tau_x2, ..., xk =   |
|                                  | tau_xk), svydesign = prob,       |
|                                  | method_outcome = "glm",          |
|                                  | family_outcome = "gaussian" )    |
|                                  | \`\`\`                           |
+----------------------------------+----------------------------------+
...
| Mass imputation based on         | \`\`\`{r, eval = FALSE} nonprob( |
| regression imputation            | outcome = y \~ x1 + x2 + ... +   |
|                                  | xk, data = nonprob, svydesign =  |
|                                  | prob, method_outcome = "glm",    |
|                                  | family_outcome = "gaussian" )    |
|                                  | \`\`\`                           |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
| Mass imputation based on nearest | \`\`\`{r, eval = FALSE} nonprob( |
| neighbour imputation             | outcome = y \~ x1 + x2 + ... +   |
|                                  | xk, data = nonprob, svydesign =  |
|                                  | prob, method_outcome = "nn",     |
|                                  | family_outcome = "gaussian",     |
|                                  | control_outcome =                |
|                                  | control_outcome(k = 2) ) \`\`\`  |
+----------------------------------+----------------------------------+
...
+----------------------------------+----------------------------------+
| Mass imputation based on         | \`\`\`{r, eval = FALSE} nonprob( |
| predictive mean matching         | outcome = y \~ x1 + x2 + ... +   |
|                                  | xk, data = nonprob, svydesign =  |
|                                  | prob, method_outcome = "pmm",    |
|                                  | family_outcome = "gaussian" )    |
|                                  | \`\`\`                           |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
| Mass imputation based on         | \`\`\`{r, eval = FALSE} nonprob( |
| regression imputation with       | outcome = y \~ x1 + x2 + ... +   |
| variable selection (LASSO)       | xk, data = nonprob, svydesign =  |
|                                  | prob, method_outcome = "pmm",    |
|                                  | family_outcome = "gaussian",     |
|                                  | control_outcome =                |
|                                  | control_out(penalty = "lasso"),  |
|                                  | control_inference =              |
|                                  | control_inf(vars_selection =     |
|                                  | TRUE) ) \`\`\`                   |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
...
+----------------------------------+----------------------------------+
| Inverse probability weighting    | \`\`\`{r, eval = FALSE} nonprob( |
| with calibration constraint with | selection = \~ x1 + x2 + ... +   |
| variable selection (SCAD)        | xk, target = \~ y, data =        |
|                                  | nonprob, svydesign = prob,       |
|                                  | method_outcome = "pmm",          |
|                                  | family_outcome = "gaussian",     |
|                                  | control_inference =              |
...
|                                  | xk, outcome = y \~ x1 + x2 +     |
|                                  | ... + xk, data = nonprob,        |
|                                  | svydesign = prob, method_outcome |
|                                  | = "glm", family_outcome =        |
|                                  | "gaussian" ) \`\`\`              |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
| Doubly robust estimator with     | \`\`\`{r, eval = FALSE} nonprob( |
| variable selection (SCAD) and    | selection = \~ x1 + x2 + ... +   |
| bias minimization                | xk, outcome = y \~ x1 + x2 +     |
|                                  | ... + xk, data = nonprob,        |
|                                  | svydesign = prob, method_outcome |
|                                  | = "glm", family_outcome =        |
|                                  | "gaussian", control_inference =  |
|                                  | control_inf( vars_selection =    |
|                                  | TRUE, bias_correction = TRUE ) ) |
|                                  | \`\`\`                           |
+----------------------------------+----------------------------------+
|                                  |                                  |
+----------------------------------+----------------------------------+
