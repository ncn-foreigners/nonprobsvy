...
+ added a second-pass Rcpp variable-selection cross-validation optimization with fused score/Jacobian evaluation, lighter CV fold handling, and three-way speed/RSS benchmark evidence versus `origin/dev` and the first Rcpp pass
+ optimized the Rcpp variable-selection cross-validation path by removing inverse-link R callbacks from hot loops, replacing explicit matrix inversion with `arma::solve()`, and adding benchmark evidence for the speedup (closes [#103](https://github.com/ncn-foreigners/nonprobsvy/issues/103))
