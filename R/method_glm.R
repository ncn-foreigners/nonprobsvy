...
#' models via the `stats::glm` function. Estimation of the mean is done using \eqn{S_{\text{P}}}
...
#'
#' (a) non-probability part  (\eqn{S_{\text{NP}}} with size \eqn{n_{\text{NP}}}; denoted as `var_nonprob` in the result)
#'
#' \deqn{
#' \hat{V}_1 = \frac{1}{n_{\text{NP}}^2}\sum_{i=1}^{n_{\text{NP}}} \hat{e}_i^2 \left\lbrace \boldsymbol{h}(\boldsymbol{x}_i; \hat{\boldsymbol{\beta}})^\prime\hat{\boldsymbol{c}}\right\rbrace^2,
#' }
#'
#' where \eqn{\hat{e}_i = y_i - m(\boldsymbol{x}_i; \hat{\boldsymbol{\beta}})} and
#' \deqn{\widehat{\boldsymbol{c}}=\left\lbrace n_{\text{NP}}^{-1} \sum_{i \in S_{\text{NP}}} \dot{\boldsymbol{m}}\left(\boldsymbol{x}_i ; \boldsymbol{\beta}^*\right) \boldsymbol{h}\left(\boldsymbol{x}_i ; \boldsymbol{\beta}^*\right)^{\prime}\right\rbrace^{-1} N^{-1} \sum_{i \in S_{\text{P}}} w_i \dot{\boldsymbol{m}}\left(\boldsymbol{x}_i ; \boldsymbol{\beta}^*\right).}
#'
#' Under the linear regression model \eqn{\boldsymbol{h}\left(\boldsymbol{x}_i ; \widehat{\boldsymbol{\beta}}\right)=\boldsymbol{x}_i} and \eqn{\widehat{\boldsymbol{c}}=\left(n_{\text{NP}}^{-1} \sum_{i \in S_{\text{NP}}} \boldsymbol{x}_i \boldsymbol{x}_i^{\prime}\right)^{-1} N^{-1} \sum_{i \in S_{\text{P}}} w_i \boldsymbol{x}_i .}
#'
#'
...
#'
#' \describe{
#'   \item{model_fitted}{fitted model either a `glm.fit`, `cv.ncvreg`, or `ncvreg` object}
...
method_glm <- function(y_nons,
  ...
  predict.ncvreg.fit <- function(object, newdata) {
    if (is.null(ncvreg_lambda)) {
      predict(object, newdata, type = "response")
    } else {
      predict(object, newdata, type = "response", lambda = ncvreg_lambda)
    }
  }

  coef.model <- function(object) {
    if (inherits(object, "ncvreg") && !is.null(ncvreg_lambda)) {
      coef(object, lambda = ncvreg_lambda)
    } else {
      coef(object)
    }
  }
  ...
  if (!vars_selection) {
    model_fitted <- stats::glm.fit(
      ...
      intercept = FALSE)
    model_family <- model_fitted$family
    ...
    if (is.null(pop_totals)) {
      ...
      y_mi_hat <- model_family$linkinv(eta)
    }
  } else {
    lambda_control <- control_outcome$lambda
    if (is.null(lambda_control)) lambda_control <- -1

    penalty_gamma <- switch(control_outcome$penalty,
    ...
    if (lambda_control == -1) {
      ...
      ncvreg_lambda <- lambda_control
      lambda_path <- c(
        if (ncvreg_lambda == 0) sqrt(.Machine$double.eps) else
          ncvreg_lambda + max(ncvreg_lambda * 1e-4, .Machine$double.eps),
        ncvreg_lambda
      )

      model_fitted <- ncvreg::ncvreg(
        X = X_nons[, -1, drop = FALSE],
        y = y_nons,
        penalty = control_outcome$penalty,
      ...
      if (is.null(pop_totals)) {
        y_nons_pred <- predict.ncvreg.fit(model_fitted, X_nons[, -1])
        y_rand_pred <- predict.ncvreg.fit(model_fitted, X_rand[, -1])
        residuals <- as.vector(y_nons - y_nons_pred)
        svydesign_updated <- stats::update(svydesign, y_hat_MI = y_rand_pred)
        svydesign_mean <- survey::svymean( ~ y_hat_MI, svydesign_updated)
        y_mi_hat <- as.numeric(svydesign_mean)
        ...
        y_rand_pred <- NA
        residuals <- as.vector(y_nons - y_nons_pred)
        eta <- drop(pop_totals %*% coef.model(model_fitted) / pop_totals[1])
        y_mi_hat <- model_family$linkinv(eta)
      }
    }
  ...
  if (se) {
    ...
    if (is.null(pop_totals)) {
      ...
      beta <- coef.model(model_fitted)
      eta_nons <- drop(X_nons %*% beta)
      eta_rand <- drop(X_rand %*% beta)

      ## is this only for the linear model?
      mx <- 1 / pop_size * colSums(X_rand * (weights(svydesign_updated) * model_family$mu.eta(eta_rand)))
      c <- MASS::ginv(1 / nrow(X_nons) * crossprod(X_nons * model_family$mu.eta(eta_nons), X_nons)) %*% mx
      ...
      mx <- 1 / pop_size * pop_totals * as.vector(model_family$mu.eta(eta_rand))
      c <- MASS::ginv(1 / nrow(X_nons) * crossprod(X_nons * model_family$mu.eta(eta_nons), X_nons)) %*% mx
      var_nonprob <- drop(1 / nrow(X_nons)^2 * crossprod(as.matrix(residuals^2), (X_nons %*% c)^2))
  ...
  return(
    structure(
      list(
        ...
        coefficients = coef.model(model_fitted),
