...
#' Euclidean distance for matching units from \eqn{S_{\text{NP}}} (non-probability) to \eqn{S_{\text{P}}} (probability).
...
#'
#' \deqn{
#' \hat{V}_1 = \frac{1}{N^2}\sum_{i=1}^{S_{\text{NP}}}\frac{1-\hat{\pi}_{\text{P}}(\boldsymbol{x}_i)}{\hat{\pi}_{\text{P}}(\boldsymbol{x}_i)}\hat{\sigma}^2(\boldsymbol{x}_i),
#' }
#'
#' where \eqn{\hat{\pi}_{\text{P}}(\boldsymbol{x}_i)} is an estimator of propensity scores which
#' we currently estimate using \eqn{n_{\text{NP}}/N} (constant) and \eqn{\hat{\sigma}^2(\boldsymbol{x}_i)} is
#' estimated using based on the average of \eqn{(y_i - y_i^*)^2}. The \eqn{y_i^*}
#' values used in this proxy are obtained by leave-one-out matching in \eqn{S_{\text{NP}}},
