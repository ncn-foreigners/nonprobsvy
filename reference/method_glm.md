...
the mean is done using \\S_B\\ probability sample or known population
totals.
...
\\\widehat{\boldsymbol{c}}=\left(n_A^{-1} \sum\_{i \in A}
\boldsymbol{x}\_i \boldsymbol{x}\_i^{\prime}\right)^{-1} N^{-1} \sum\_{i
\in B} w_i \boldsymbol{x}\_i .\\
